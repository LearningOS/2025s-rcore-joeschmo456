...
pub fn sys_sleep(ms: usize) -> isize {
    debug!("kernel:pid[{}] tid[{}] sys_sleep", get_pid(), get_tid());
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    let pid = get_pid();
    let tid = get_tid();
    debug!("kernel:pid[{}] tid[{}] sys_semaphore_up", pid, tid);
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    let pid = get_pid();
    let tid = get_tid();
    debug!("kernel:pid[{}] tid[{}] sys_semaphore_down", pid, tid);
    let process = current_process();
    let try_down = {
        let mut process_inner = process.inner_exclusive_access();
    ...
    if let Some(sem) = try_down {
        ...
        info!("semaphore {} down", sem_id);
        {
            current_process()
                .inner_exclusive_access()
                .request_semaphore(tid, sem_id, 1);
        }
        return 0;
    } else {
        ...
        debug!(
            "???{} -> {:?}",
            tid,
            current_process().inner_exclusive_access().banker
        );
        return -0xdead;
    }
}
